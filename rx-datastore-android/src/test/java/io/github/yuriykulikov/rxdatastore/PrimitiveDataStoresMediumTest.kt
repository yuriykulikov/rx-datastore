...
class PrimitiveDataStoresMediumTest(val name: String, private val inMemory: Boolean) {
    ...
    private var stores: PrimitiveDataStoreFactory = if (inMemory) {
        InMemoryRxDataStoreFactory()
    } else {
        SharedRxDataStoreFactory.createFromSharedPreferences(stubSharedPreferences(), mockk(relaxed = false))
    }
    ...
    private fun stubSharedPreferences(): SharedPreferences {
        return mockk {
            ...
            every { getInt(any(), any()) } answers {
                values.getOrElse(firstArg()) { lastArg() } as Int
            }
